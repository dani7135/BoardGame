...
- Create Angular project with ng: `ng new boardgame.client --defaults --skip-install --skip-git `.
...
- Update `app.component.ts` component to fetch and display information.
- Modify `app.component.spec.ts` with updated tests.
- Update `app.module.ts` to import the HttpClientModule.
- Create project file (`boardgame.client.esproj`).
- Create `launch.json` to enable debugging.
- Create `nuget.config` to specify location of the JavaScript Project System SDK (which is used in the first line in `boardgame.client.esproj`).
